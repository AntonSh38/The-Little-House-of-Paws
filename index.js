import{a as p,b as R,i as c,S as y,N as v,P as h,K as B,R as U}from"./assets/vendor-DvvmSQtX.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const n of r.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&a(n)}).observe(document,{childList:!0,subtree:!0});function t(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(o){if(o.ep)return;o.ep=!0;const r=t(o);fetch(o.href,r)}})();async function N(){return(await p.get("https://paw-hut.b.goit.study/api/categories")).data}async function O(e,s,t){return(await p.get("https://paw-hut.b.goit.study/api/animals",{params:{page:e,limit:s,categoryId:t}})).data}const P=document.querySelector(".pets-card-list");P.addEventListener("click",e=>{if(e.target.nodeName!=="BUTTON")return;const s=e.target.closest(".pets-card-items"),t=JSON.parse(decodeURIComponent(s.getAttribute("data-animal"))),a=R.create(`
  ...
  `,{closable:!0,onShow:r=>{r.element().querySelector(".modal-pet-btn-close").addEventListener("click",()=>r.close()),document.body.style.overflow="hidden",document.addEventListener("keydown",o)},onClose:()=>{document.body.style.overflow="",document.removeEventListener("keydown",o)}});function o(r){r.key==="Escape"&&a.close()}a.show()});const w=document.querySelector(".loader");function L(){w.classList.remove("is-hidden")}function E(){w.classList.add("is-hidden")}const x=document.querySelector(".pets-nav"),S=document.querySelector(".pets-card-list"),d=document.querySelector(".pets-btn-more");let u=1,$=window.innerWidth<1440?8:9,k=null,q=0;H();b();x.addEventListener("click",e=>{if(e.target.nodeName==="LI"){for(const s of e.target.parentElement.children)s.classList.remove("active");e.target.classList.add("active"),k=e.target.getAttribute("data-id"),S.innerHTML="",u=1,b()}});d.addEventListener("click",()=>{u+=1,b(),d.blur()});async function H(){try{const s=(await N()).map(t=>`<li class="pets-nav-item" data-id="${t._id}">${t.name}</li>`).join("");x.insertAdjacentHTML("beforeend",s)}catch{c.error({title:"",message:"Наші пухнастики сперечаються, хто буде в якій категорії 😸. Повернемо їх незабаром!",position:"topRight"})}}async function b(){try{L();const e=await O(u,$,k);M(e.animals),q=e.totalItems,I()}catch{c.error({title:"",message:"Ой! Наші пухнастики сховалися 🐾. Спробуйте оновити сторінку або поверніться пізніше.",position:"topRight"})}finally{E()}}function I(){q<=u*$?d.classList.add("hide-pets-btn"):d.classList.remove("hide-pets-btn")}function M(e){const s=e.map(t=>`
